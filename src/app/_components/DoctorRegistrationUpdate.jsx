...
let handelchangeInput=(e)=>{
   ...
   data={...data,phone:Number.parseInt(data.phone)};
     ...
     //   get specific Doctor detail get req
   let handelDoctorRegUpdate=async()=>{
    try{ 
          let res= await fetch(SummeryApi.updateDoctorDetail.url+'/'+`${user?._id}`,{
      ...
      let result= await res.json();
        ...
        if(result.success){
            setData(result.data.doctorReg);

            // toast.success('doctor reg sucesfull');
          router.push('/')
          // console.log(result.data.doctorReg);
        }
      
    }
    catch(e){
        console.log('data not fetchs in frontened',e)

    }
}

useEffect(()=>{
    handelDoctorRegUpdate();
 },[])   
...
// delete doctor registration and role become general
let handelDeleteReg= async(e)=>{
    try{ 
    e.preventDefault();
   //  retailer is in data base all signup detail and extra field add retailer:{data}
      user={...user, doctorReg:{}}
   //  console.log(user);
     let res= await fetch(SummeryApi.deleteDoctorDetail.url+'/'+`${user?._id}`,{
       method:SummeryApi.deleteDoctorDetail.method,
       headers:{
          'Content-Type':'application/json'
       },
        body:JSON.stringify(user)
...
//  To  certificate   image  of uplode of doctor
...
let HandelZoomUploadImgDoctor=(index)=>{
  ...
  return (
    ...
    <div className='bg-white rounded absolute top-[20px] left-10 md:left-[20vw]  p-4 h-[30rem] w-[85%] md:h-[33rem] md:w-[60%] overflow-x-hidden' style={{backgroundColor:'white !important'}}>
      ...
      <div className='flex justify-center w-[100%]'>  <h1 className='text-2xl font-semibold '> Update Doctors Registration </h1> </div> 
   ...
   <div className='text-red-600 absolute bottom-0 right-0 text-2xl bg-white ' onClick={()=>{handelDeleteImg(index)}} >
     <MdDelete/>
          ...
          <div className='flex flex-col items-start '> 
           ...
           <button type='submit' className='px-4 w-[100%] py-1.5 bg-blue-500 text-white self-center rounded-full my-3 text-center'  onClick={handelNewDoctorReg}>Update Doctor Details</button> 
       </form>
       <div className='w-[100%] flex justify-end'>
       <button type='submit' className='px-4   py-1.5 bg-red-500 text-white   rounded-full my-3 mr-[4rem]'  onClick={handelDeleteReg}>Delete Doctor Registration</button> 
</div>
</div>
{ShowBigimg &&
